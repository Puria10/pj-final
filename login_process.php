...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username) || empty($password)) {
        ...
        if ($connect) {
            $query = $connect->prepare("SELECT `username`, `password`, `role` FROM `user` WHERE `username` = ?");
            ...
            if ($result->num_rows > 0) {
                ...
                if ($password === $user['password']) {
                    ...
                    if ($user['role'] === 'admin') {
                        echo "<p class='alert alert-success'>ورود موفقیت‌آمیز! خوش آمدید, $username. به داشبورد ادمین خوش آمدید.</p>";
